...
exports.adminLogin = async (userName, password) => {
  ...
  return { admin: adminWithoutPassword, token, message: "Admin loggedIn successfully" };
};

exports.adminAddCustomer = async(userName, password, name, email) => {
 let customer = await Customer.findOne({userName});
 if(customer){
  return {customer: null, message: 'Customer already exist with this username'};
 }
 customer = await Customer.findOne({email});
 if(customer){
  return {customer: null, message: 'Customer already exist with this email'};
 }
...
exports.adminAddEngineer = async(userName, password, name, email) => {
 ...
 if(engineer){
  return {engineer: null, message: 'Engineer already exist with this username'};
 }
 engineer = await Engineer.findOne({email});
 if(engineer){
  return {engineer: null, message: 'Engineer already exist with this email'};
 }
 const salt = await bcrypt.genSalt(10);
 const hashedPassword = await bcrypt.hash(password, salt);
